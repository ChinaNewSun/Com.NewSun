    ...
    public class MongoDBHelper<Model, IdT> where Model : BaseModel, new()
    {
        ...
        public MongoDBHelper()
        {
            if (mongoDBConnectionStr == "")
                mongoDBConnectionStr = System.Configuration.ConfigurationManager.AppSettings["MongoDBConnectionStr"];
            if (mongoServer == null)
                mongoServer = MongoDB.Driver.MongoServer.Create(mongoDBConnectionStr);
            if (mongoDBName == "")
                mongoDBName = System.Configuration.ConfigurationManager.AppSettings["MongoDatabaseName"];
